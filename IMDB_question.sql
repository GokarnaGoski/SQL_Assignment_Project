...
SELECT  count(id) as number_of_moviesyear, year
FROM movie
GROUP BY year
ORDER BY year;

SELECT EXTRACT(MONTH FROM date_published) AS release_month, count(id) as number_of_movies
FROM movie
...
JOIN ratings AS r
ON m.id = r.movie_id
WHERE country IN ('USA', 'United States', 'U.S.A') 
...
SELECT production_company, SUM(r.total_votes) AS total_votes_received
FROM movie AS m
...
SELECT name AS actor_name, SUM(r.total_votes) AS total_votes, COUNT(m.id) AS movie_count,
...
JOIN ratings AS r ON rm.movie_id = r.movie_id
...
SELECT name AS actress_name, SUM(r.total_votes) AS total_votes, COUNT(m.id) AS movie_count,
...
SELECT genre, AVG(m.duration) AS avg_duration,
    SUM(AVG(duration)) OVER (PARTITION BY genre ORDER BY genre ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_total_duration,
    AVG(AVG(duration)) OVER (PARTITION BY genre ORDER BY genre ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS moving_avg_duration
FROM movie AS m
JOIN genre AS g 
ON m.id = g.movie_id
GROUP BY g.genre
...
SELECT name AS actress_name, SUM(r.total_votes) AS total_votes,
    COUNT(DISTINCT m.id) AS movie_count,
    ROUND(SUM(r.avg_rating * r.total_votes) / SUM(r.total_votes), 4) AS actress_avg_rating,
    RANK() OVER (ORDER BY ROUND(SUM(r.avg_rating * r.total_votes) / SUM(r.total_votes), 4) DESC, 
            SUM(r.total_votes) DESC, name ASC) AS actress_rank
FROM movie AS m
JOIN ratings AS r 
ON m.id = r.movie_id
JOIN genre AS g 
...
SELECT 
    ...
    ROUND(AVG(DATEDIFF(m.date_published, sub.prev_date)), 0) AS avg_inter_movie_days,
...
LEFT JOIN (
    SELECT 
        ...
        m1.id, 
        m1.date_published, 
        LAG(m1.date_published) OVER (PARTITION BY d1.name_id ORDER BY m1.date_published) AS prev_date
    FROM director_mapping d1
    JOIN movie m1 ON d1.movie_id = m1.id
) sub ON sub.id = m.id AND sub.name_id = d.name_id
